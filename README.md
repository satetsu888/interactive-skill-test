...
```bash
npx skills add satetsu888/interactive-skill-test --skill item-review
npx skills add satetsu888/interactive-skill-test --skill kanban
npx skills add satetsu888/interactive-skill-test --skill grid
...
```bash
node skills/item-review/server.mjs --port 5190 --data '{
...
```bash
node skills/kanban/server.mjs --port 5190 --data '{
...
```bash
node skills/grid/server.mjs --port 5190 --data '{
...
```bash
node skills/color-palette/server.mjs --port 5190 --data '{
...
```bash
node skills/image-annotate/server.mjs --port 5190 --data '{
...
```bash
node skills/datafilter/server.mjs --port 5190 --data '{
...
```bash
node skills/pairwise/server.mjs --port 5190 --data '{
...
```bash
node skills/quiz/server.mjs --port 5190 --data '{
...
```bash
node skills/planreview/server.mjs --port 5190 --file /path/to/plan.md --title "Plan Review"
```

### Code Review Review Skill - コードレビュー

AIによるコードレビューの結果を GitHub 風の差分 UI で表示し、レビューコメントに対してユーザーが accept / deny を選択できます。

![Code Review Review Skill](docs/screenshots/codereview-review-skill.png)

```bash
node skills/codereview-review/server.mjs --port 5190 --data '{
  "title": "Review: Add feature",
  "commit": "HEAD~1",
  "repo": "/path/to/repo",
  "comments": [
    { "id": "c1", "file": "src/app.ts", "line": 14, "body": "この関数にエラーハンドリングを追加すべきです" }
  ]
}'
```
...
```bash
node skills/diffmerge/server.mjs --port 5190 --data '{
  "title": "文章のリライト比較",
...
```
src/
├── item-review/           # Demo Skill (Approve/Reject UI)
├── kanban/         # Kanban Skill (Drag & Drop board)
├── grid/           # Grid Skill (Cell selector)
├── color-palette/          # Color Skill (Palette builder)
├── image-annotate/       # Annotate Skill (Image annotation)
...
skills/
├── item-review/     # Built output + server
├── kanban/
├── grid/
├── color-palette/
├── image-annotate/
